...
struct TrendingProvider: TimelineProvider {
    ...
    func getSnapshot(in context: Context, completion: @escaping (ItemContentEntry) -> Void) {
        Task {
    ...
    func getTimeline(in context: Context, completion: @escaping (Timeline<ItemContentEntry>) -> Void) {
        if context.isPreview {
...
struct CronicaTrendingWidgetEntryView: View {
    var entry: ItemContentEntry
...
struct CronicaTrendingWidget: Widget {
    let kind: String = WidgetKind.trending

    var body: some WidgetConfiguration {
        StaticConfiguration(kind: kind, provider: TrendingProvider()) { entry in
            CronicaTrendingWidgetEntryView(entry: entry)
        }
